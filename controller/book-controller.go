...
//BookController é um contrato do que bookController pode fazer
...
//NewBookController cria uma nova instancia de BoookController
...
func (c *bookController) Update(context *gin.Context) {
	...
	if c.bookService.IsAllowedToEdit(userID, bookUpdateDTO.ID) {
		...
		response := helper.BuildErrorResponse("Você não tem permissão", "Você não é o dono", helper.EmptyObj{})
...
func (c *bookController) Delete(context *gin.Context) {
	...
	id, err := strconv.ParseUint(context.Param("id"), 0, 0)
	if err != nil {
		response := helper.BuildErrorResponse("Falha ao obter id", "Nenhum id de parâmetro foi encontrado", helper.EmptyObj{})
